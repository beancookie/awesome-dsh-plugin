...
- [xiaoyuyu6420/dsh-backup](https://github.com/xiaoyuyu6420/dsh-backup) - One-command backup and restore of DSH user data: scheduled backups with daily/weekly tiered retention, sha256 verification, automatic pre-upgrade snapshots and one-shot plugin update, a session-log doctor plus a read-only upgrade migration pre-check, a rescue console that works when the host will not boot, and credential redaction by default.
